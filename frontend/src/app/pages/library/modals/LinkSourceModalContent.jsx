...
import { useLinkPersonSourceMutation, useUnlinkPersonSourceMutation, useSetPrimaryPersonSourceMutation } from '@/queries/libraryQueries';
...
const FemaleSilhouette = () => (
  <svg viewBox="0 0 24 24" className="gender-silhouette gender-silhouette--female" fill="currentColor">
...
const MaleSilhouette = () => (
  <svg viewBox="0 0 24 24" className="gender-silhouette gender-silhouette--male" fill="currentColor">
...
const OtherSilhouette = () => (
  <svg viewBox="0 0 24 24" className="gender-silhouette gender-silhouette--other" fill="currentColor">
    <circle cx="12" cy="8" r="4" />
    <path d="M12 14c-6.1 0-10 4-10 8h20c0-4-3.9-8-10-8zm-7.9 6c.9-2.5 4-4 7.9-4s7 1.5 7.9 4H4.1z" />
  </svg>
);

export default function LinkSourceModalContent({ personId, defaultQuery = '', person: initialPerson, onClose }) {
  ...
  if (activeSearchSource) {
    return (
      <div className="link-source-modal link-source-modal--search-view">
        <div className="link-source-modal__header">
          <NavButton
            onClick={() => {
            ...
            className="link-source-modal__back-button"
          >
            Back to Sources
          </NavButton>
          <span className="link-source-modal__search-title">
        ...
        <div className="link-source-modal__results">
          {isSearching ? (
            ...
            <div className="link-source-modal__placeholder">
              Type a performer's name and search to link metadata.
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="link-source-modal link-source-modal--grid-view">
          ...
          return (
            <div key={bucket.key} className={`link-source-card ${isLinked ? 'link-source-card--linked' : 'link-source-card--unlinked'} ${isPrimary ? 'link-source-card--primary' : ''}`}>
              ...
              <div className="link-source-card__avatar-wrapper">
                {isLinked ? (
                  profileImg ? (
                    <img src={profileImg} alt={person.name} className="link-source-card__avatar-img" />
                  ) : (
                    <div className="link-source-card__avatar-placeholder">
                      <User size={32} />
                    </div>
                  )
                ) : (
                  <div className="link-source-card__silhouette">
                    {renderSilhouette(person?.gender)}
                  </div>
                )}
              </div>

              <div className="link-source-card__info">
                {isLinked ? (
                  <>
                    <div className="link-source-card__name">{person.name}</div>
                    <div className="link-source-card__ext-id" title={linkedInfo.external_id}>
                      ID: {linkedInfo.external_id}
                    </div>
                  </>
                ) : (
                  <div className="link-source-card__unlinked-text">Not Connected</div>
                )}
              </div>
              ...
              <div className="link-source-card__actions">
                {isLinked ? (
                  ...
                  <Button
                    ...
                    icon={Search}
                    className="link-source-card__link-btn"
                  >
                    Link Profile
                  </Button>
                )}
