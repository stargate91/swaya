...
from app.core.enums import Provider, MediaType, ItemStatus, CustomListType, ActionStatus, ExtraCategory
                    ...
                    if active_m.media_type == MediaType.EPISODE:
                        ...
                        if season_m:
                            ...
                            if loc:
                                ...
                                resolved = self.img_service.resolve_image_url(poster_path, "posters")
                                if resolved:
                        ...
                        if not images_list:
                            still_path = active_m.local_still_path or active_m.still_path
                            resolved = self.img_service.resolve_image_url(still_path, "stills")
                            if resolved:
                                images_list.append({"path": resolved})
                    elif active_m.media_type == MediaType.SCENE:
                        backdrop_path = active_m.local_backdrop_path or active_m.backdrop_path
                        resolved = self.img_service.resolve_image_url(backdrop_path, "scene_stills")
                        if resolved:
                            images_list.append({"path": resolved})
                        else:
